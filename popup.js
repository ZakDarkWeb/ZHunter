...
function scrapePageData() {
  try {
    ...
    const addImgs = (selector, limit = 14) => document.querySelectorAll(selector).forEach(i => {
      let src = i.getAttribute('data-zoom-image') || i.getAttribute('data-old-hires') || i.getAttribute('data-src') || i.getAttribute('data-image') || i.getAttribute('src') || i.src || '';
      if (!src && i.getAttribute('srcset')) src = i.getAttribute('srcset').split(',').pop().trim().split(/\s+/)[0];
      src = abs(src).replace(/\._[A-Z0-9_,]+_\./, '._AC_SL1500_.');
    ...
    if (h.includes('amazon.')) {
      ...
      addImgs('[data-testid="product-image"] img, .sc-product-image img, picture img, img[src*="scene7.samsclub.com"], [class*="gallery"] img, [class*="thumbnail"] img');
    ...
    if (data.images.length < 2) {
      // Only grab large images (product images are typically big)
      document.querySelectorAll('img').forEach(i => {
        ...
        let src = i.getAttribute('data-zoom-image') || i.getAttribute('data-old-hires') || i.getAttribute('data-src') || i.getAttribute('src') || i.src || '';
        if (!src) return;
        src = abs(src);
...
function initTabs() {
  document.querySelectorAll('.tab-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (id === 'images' && typeof ImgTabState !== 'undefined' && !ImgTabState.images.length && !ImgTabState.isScanning) {
        setTimeout(() => scanPageImages(), 120);
      }
...
function updateStatsDashboard() {
  ...
  if (chrome.storage && chrome.storage.local && chrome.storage.local.getBytesInUse) {
    chrome.storage.local.getBytesInUse(null, bytes => {
      ...
      if (sizeEl) {
        sizeEl.textContent = `${size} ${unit}`;
        const mb = (bytes || 0) / (1024 * 1024);
        sizeEl.style.color = mb > 100 ? '#ef4444' : (mb > 50 ? '#f59e0b' : '');
        sizeEl.style.fontWeight = mb > 50 ? 'bold' : '';
      }
    });
  }
}
...
function initCollectTab() {
  ...
  if (State.inputMode === 'auto') huntCurrentTab();
...
// ── Import products from CSV / XLSX / JSON ───────────────────────────────────
async function doImport(file) {
  ...
  try {
    if (ext === 'json') {
      // ── JSON import (ZHunter backup format) ──────────────────────────────
      const text = await file.text();
      const parsed = JSON.parse(text);
      const raw = Array.isArray(parsed) ? parsed : (parsed.links || []);
      links = raw.filter(l => l && l.url);

    } else if (ext === 'csv') {
      // ── CSV import ────────────────────────────────────────────────────────
      const text = await file.text();
      const rows = text.split(/\r?\n/).filter(Boolean);
      if (rows.length < 2) { toast('CSV has no data rows', 'warn'); return; }
      ...
      const urlCol   = colIdx(['url', 'link', 'product url', 'product_url']);
      const titleCol = colIdx(['title', 'name', 'product', 'product title', 'product_title']);
      const priceCol = colIdx(['price', 'sale price', 'sale_price']);
      const platCol  = colIdx(['platform', 'site', 'source']);
      const notesCol = colIdx(['notes', 'note', 'description', 'desc']);
      const folderCol= colIdx(['folder', 'category', 'group']);
      const tagsCol  = colIdx(['tags', 'tag', 'labels']);

      if (urlCol < 0) { toast('CSV must have a "URL" column', 'err'); return; }
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        const url = (cells[urlCol] || '').trim().replace(/^"|"$/g, '');
        if (!url || !isValidURL(url)) continue;
        links.push({
          id:       `m_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,8)}`,
          url,
          title:    titleCol >= 0 ? (cells[titleCol] || '').replace(/^"|"$/g, '').trim() : '',
          price:    priceCol >= 0 ? (cells[priceCol] || '').replace(/^"|"$/g, '').trim() : '',
          platform: platCol  >= 0 ? (cells[platCol]  || '').replace(/^"|"$/g, '').trim() : '',
          notes:    notesCol >= 0 ? (cells[notesCol] || '').replace(/^"|"$/g, '').trim() : '',
          folder:   folderCol>= 0 ? (cells[folderCol]|| '').replace(/^"|"$/g, '').trim() || 'General' : 'General',
          tags:     tagsCol  >= 0 ? (cells[tagsCol]  || '').replace(/^"|"$/g, '').split(/[;|]/).map(t=>t.trim()).filter(Boolean) : [],
          images:   [],
          videos:   [],
          variants: [],
      ...
      const findVal = (row, names) => {
        for (const k of Object.keys(row)) {
          if (names.some(n => norm(k) === n)) return String(row[k] || '').trim();
        }
        return '';
      };

      for (const row of rows) {
        const url = findVal(row, ['url','link','producturl']);
        if (!url || !isValidURL(url)) continue;
        const tagsRaw = findVal(row, ['tags','tag','labels']);
        links.push({
          id:       `m_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,8)}`,
          url,
          title:    findVal(row, ['title','name','productname','producttitle']) || '',
          price:    findVal(row, ['price','saleprice','currentprice']) || '',
          platform: findVal(row, ['platform','site','source']) || '',
          notes:    findVal(row, ['notes','note','description']) || '',
          folder:   findVal(row, ['folder','category','group']) || 'General',
    ...
    toast('Import failed: ' + (err.message || 'Unknown error'), 'err');
    return;
  }

  if (links.length === 0) { toast('No valid products found in file', 'warn'); return; }

  // Merge into master sheet — skip duplicates by URL
  const data = await loadData();
  const existingUrls = new Set((data.links || []).map(l => l.url));
  const newLinks = links.filter(l => !existingUrls.has(l.url));
  const dupes = links.length - newLinks.length;

  let importedCount = 0;
  for (const link of newLinks) {
    const saved = await msg({
      action: 'ADD_LINK',
      url: link.url,
      title: link.title,
      folder: link.folder || 'General',
      tags: link.tags || [],
      notes: link.notes || '',
      price: link.price || '',
    ...
    if (saved?.success) importedCount++;
  }
  await refresh();

  const msg = importedCount + ' product' + (importedCount !== 1 ? 's' : '') + ' imported';
  toast(msg + (dupes > 0 ? ' (' + dupes + ' duplicates skipped)' : ''), 'ok');
}
...
function doExportJson() {
  ...
  const clean = State.data.links.map(l => ({
    ...l,
    // Keep real URLs; only strip inline base64 data URIs (they're huge and non-restorable)
    images: (l.images || []).map(img =>
      (typeof img === 'string' && img.startsWith('data:')) ? '[base64_image_omitted]' : img
    ),
...
function exportLinksAsHtml(linksArr, filename) {
  ...
  const folderSections = Object.entries(byFolder).map(([folder, fLinks]) => {
    const cards = fLinks.map(l => {
      ...
      // "Download All Images" button — sequential download of every image in this product
      const downloadableImgs = images.filter(s => typeof s === 'string' && s.length > 0);
      const safeName = (l.title || 'product').substring(0, 20).replace(/[^a-zA-Z0-9]/g, '_');
      // Unicode-safe: encode at template time, decode in onclick handler
      const _dlB64 = btoa(unescape(encodeURIComponent(JSON.stringify(downloadableImgs))));
      const dlAllBtn = downloadableImgs.length > 0
        ? `<button class="btn btn-dl" onclick="downloadCardImages(this, JSON.parse(decodeURIComponent(escape(atob('${_dlB64}')))), '${safeName}')">⬇ Download All Images (${downloadableImgs.length})</button>`
        : '';
...
async function downloadCardImages(btn, urls, safeName) {
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    const filename = (safeName || 'product') + '_img' + (i+1) + '.' + ext;

    let downloaded = false;
    try {
      // Route through background.js — service worker has no CORS restrictions.
      // The popup fetch context is blocked by CDN CORS headers (Walmart, Amazon, Temu).
      const res = await chrome.runtime.sendMessage({
        action: 'FETCH_BASE64',
        url: urls[i],
        settings: { imageFormat: 'original' }  // preserve quality, no canvas conversion
      });
      if (res?.success && res?.base64) {
        // Convert data-URL base64 back to Blob for clean download
        const [header, b64] = res.base64.split(',');
...
function initHuntPreviewModal() {
  ...
  document.getElementById('modalAiBtn')?.addEventListener('click', async () => {
    ...
    if (res?.success) {
      ...
      alert('AI failed: ' + (res?.error || 'Unknown error'));
    }
  });
...
async function handleSaveEdit() {
  ...
  // Read existing link to preserve fields not shown in the edit form (e.g. videos array)
  const existingLink = State.data.links.find(l => l.id === id) || {};
  ...
  const updates = {
    ...
    videos:   existingLink.videos || []   // preserve scraped videos array
  };
...
// ============================================================
// IMAGE TAB MODULE — Dedicated image scanning + download
// ============================================================

const ImgTabState = {
  images:        [],    // [{url, selected, failed}]
  isScanning:    false,
  currentTabUrl: '',
  currentAsin:   null
};
...
function initImagesTab() {
  $('imgHuntBtn')?.addEventListener('click', scanPageImages);
  $('imgRefreshBtn')?.addEventListener('click', scanPageImages);
  $('imgSelectAllBtn')?.addEventListener('click', () => {
    ImgTabState.images.forEach(img => { if (!img.failed) img.selected = true; });
    renderImgTabGrid();
    updateImgFoundCount();
  });
  $('imgDeselectAllBtn')?.addEventListener('click', () => {
    ImgTabState.images.forEach(img => img.selected = false);
    renderImgTabGrid();
    updateImgFoundCount();
  });
  $('imgDownloadBtn')?.addEventListener('click', downloadSelectedImages);
  $('imgCopyAllUrlsBtn')?.addEventListener('click', copyAllImageUrls);

  // Auto-scan if images tab is initially active
  if ($('tab-images')?.classList.contains('active')) {
    setTimeout(() => scanPageImages(), 500);
  }
}

async function scanPageImages() {
  if (ImgTabState.isScanning) return;
  ImgTabState.isScanning = true;

  const huntBtn       = $('imgHuntBtn');
  const scanningState = $('imgScanningState');
  const emptyState    = $('imgEmptyState');
  const statusBar     = $('imgStatusBar');

  if (huntBtn)       huntBtn.disabled = true;
  if (scanningState) scanningState.classList.remove('hidden');
  if (emptyState)    emptyState.style.display = 'none';
  if (statusBar)     statusBar.textContent = 'Scanning for images…';

  try {
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (!tab?.id) throw new Error('no_tab');

    ImgTabState.currentTabUrl = tab.url || '';
    ImgTabState.currentAsin   = extractAsin(tab.url || '');
    ...
    ImgTabState.images = rawImages
      .filter(u => u && typeof u === 'string' && u.startsWith('http'))
      .map(url => ({ url, selected: true, failed: false }));

    renderImgTabGrid();
    updateImgFoundCount();

    const count = ImgTabState.images.length;
    if (statusBar) statusBar.textContent = '';
    if (count === 0) {
      if (emptyState) emptyState.style.display = 'flex';
      toast('No images found on this page', 'info');
    } else {
      toast(`Found ${count} image${count !== 1 ? 's' : ''}! 🖼️`, 'ok');
    }
  } catch (e) {
    if (emptyState) emptyState.style.display = 'flex';
    ...
    ImgTabState.isScanning = false;
    if (huntBtn)       huntBtn.disabled = false;
    if (scanningState) scanningState.classList.add('hidden');
  }
}

function renderImgTabGrid() {
  const grid       = $('imgGrid');
  const emptyState = $('imgEmptyState');
  if (!grid) return;
  Array.from(grid.children).forEach(c => { if (c.id !== 'imgEmptyState') c.remove(); });
  if (!ImgTabState.images.length) {
    if (emptyState) emptyState.style.display = 'flex';
    return;
  }
  if (emptyState) emptyState.style.display = 'none';
  const frag = document.createDocumentFragment();
  ImgTabState.images.forEach((imgObj, idx) => frag.appendChild(buildImgTabCard(imgObj, idx)));
  grid.appendChild(frag);
}

function buildImgTabCard(imgObj, idx) {
  const card = document.createElement('div');
  card.className = 'img-tab-card' + (imgObj.selected ? ' selected' : '') + (imgObj.failed ? ' failed' : '');
  card.dataset.idx = idx;

  const imgEl     = document.createElement('img');
  imgEl.className = 'img-tab-card-img';
  imgEl.src       = imgObj.url;
  imgEl.alt       = `Image ${idx + 1}`;
  imgEl.loading   = 'lazy';
  imgEl.dataset.originalUrl = imgObj.url;
  imgEl.onerror = () => {
    imgObj.failed = true; imgObj.selected = false;
    card.classList.add('failed'); card.classList.remove('selected');
    updateImgFoundCount();
  };

  const check     = document.createElement('div');
  check.className = 'img-tab-check';
  check.innerHTML = `<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg>`;

  const badge     = document.createElement('div');
  badge.className = 'img-tab-badge';
  badge.textContent = idx + 1;

  card.append(imgEl, check, badge);
  card.addEventListener('click', () => {
    if (imgObj.failed) return;
    imgObj.selected = !imgObj.selected;
    card.classList.toggle('selected', imgObj.selected);
    updateImgFoundCount();
  });
  return card;
}

function updateImgFoundCount() {
  const el = $('imgFoundCount');
  if (!el) return;
  const total    = ImgTabState.images.length;
  const selected = ImgTabState.images.filter(i => i.selected && !i.failed).length;
  el.textContent = total > 0 ? `${selected}/${total} selected` : '0 images';
}

async function downloadSelectedImages() {
  const selected = ImgTabState.images.filter(i => i.selected && !i.failed);
  if (!selected.length) { toast('No images selected', 'warn'); return; }

  const btn       = $('imgDownloadBtn');
  const statusBar = $('imgStatusBar');
  const format    = $('imgFormatSel')?.value || 'original';

  if (btn) btn.disabled = true;
  if (statusBar) statusBar.textContent = `Downloading ${selected.length} image${selected.length !== 1 ? 's' : ''}…`;

  const baseName = ImgTabState.currentAsin || 'product';
  let saved = 0;

  for (let i = 0; i < selected.length; i++) {
    const imgObj = selected[i];
    if (statusBar) statusBar.textContent = `Downloading ${i + 1} of ${selected.length}…`;
    try {
      const urlPath = imgObj.url.split('?')[0];
      let ext = (urlPath.split('.').pop().toLowerCase().match(/^[a-z0-9]+/) || ['jpg'])[0];
      if (!['jpg','jpeg','png','webp','gif','avif'].includes(ext)) ext = 'jpg';
      if (format === 'png') ext = 'png';
      else if (format === 'jpg') ext = 'jpg';
      const filename = `${baseName}_img${String(i + 1).padStart(2, '0')}.${ext}`;
      const a = document.createElement('a');
      a.href = imgObj.url; a.download = filename;
      a.style.display = 'none';
  ...
  if (statusBar) statusBar.textContent = `Downloaded ${saved} image${saved !== 1 ? 's' : ''}`;
  toast(`Downloaded ${saved} image${saved !== 1 ? 's' : ''}! 📥`, 'ok');
}

async function copyAllImageUrls() {
  const valid = ImgTabState.images.filter(i => !i.failed).map(i => i.url);
  if (!valid.length) { toast('No image URLs to copy', 'warn'); return; }
  try {
    await navigator.clipboard.writeText(valid.join('\n'));
    toast(`${valid.length} image URL${valid.length !== 1 ? 's' : ''} copied!`, 'ok');
  } catch { toast('Failed to copy image URLs', 'err'); }
}

// ============================================================
// VIDEO TAB MODULE — One-click video download (Asincrate-style)
// ============================================================

const VidTabState = {
  videos:        [],    // [{url, type, label, thumbnail, ytId}]
  isScanning:    false,
  currentTabUrl: '',
  currentAsin:   null
};
...
function classifyVideo(url) {
  ...
  // YouTube — extract video ID inline
...
async function scanVideos() {
  ...
  try {
    ...
    VidTabState.currentAsin   = extractAsin(tab.url || '');
...
// Render-session counter: incremented each time renderVideoCards() runs,
// so element IDs like vp-${uid}_0 are unique even across multiple renders.
...
function buildVideoCard(video, idx, seed) {
  ...
  const TYPE_CLASS = {
    ...
    youtube: 'vid-badge-youtube', direct: 'vid-badge-direct', unknown: 'vid-badge-unknown'
  };
  const card = document.createElement('div');
  card.className = 'vid-card'; card.dataset.idx = idx;
  const thumbHTML = video.type === 'youtube' && video.thumbnail
    ? `<img class="vid-thumb-img" src="${esc(video.thumbnail)}" alt="Thumbnail" loading="lazy" onerror="this.parentElement.innerHTML='<div class=vid-thumb-icon>&#9654;</div>'">`
  ...
  card.innerHTML = `
    ...
    <div class="vid-thumbnail-area">${thumbHTML}</div>
...
async function downloadVideo(video, uid) {
  ...
  try {
    if (video.type === 'youtube') {
      ...
      toast('YouTube opened in new tab (direct DL not supported)', 'info');
      return;
    }
    ...
    if (video.type === 'hls') {
      ...
      const a = document.createElement('a');
      a.href = objUrl; a.download = `${base}.ts`;
      document.body.appendChild(a); a.click();
      setTimeout(() => { URL.revokeObjectURL(objUrl); a.remove(); }, 1000);
      toast('HLS video saved as .ts ✅', 'ok');
    } else {
      setP(30, 'Downloading…');
      ...
      if (!downloaded) {
        const a = document.createElement('a');
        a.href = video.url; a.download = filename;
        document.body.appendChild(a); a.click();
...
// HLS stream downloader — fetches master playlist, picks highest quality,
// downloads all .ts segments in parallel batches, concatenates into one blob.
async function downloadHLSStream(m3u8Url, progressCb) {
    ...
    if (text.includes('#EXT-X-STREAM-INF')) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (nxt && !nxt.startsWith('#') && bw > bestBw) {
          bestBw = bw;
          bestVariant = nxt.startsWith('http') ? nxt : baseUrl + nxt;
        }
      }
  ...
  for (let i = 0; i < segments.length; i += CONCUR) {
    ...
    const results = await Promise.all(
      batch.map(async (segUrl, bi) => {
        const r = await fetch(segUrl, { credentials: 'omit' });
        if (!r.ok) throw new Error(`Segment ${i + bi} failed: HTTP ${r.status}`);
...
async function copyAllVideoUrls() {
  ...
  try {
    ...
    toast(`${VidTabState.videos.length} video URL${VidTabState.videos.length !== 1 ? 's' : ''} copied! 📋`, 'ok');
  } catch { toast('Failed to copy URLs', 'err'); }
}

async function downloadAllVideosAsZip() {
  if (!VidTabState.videos.length) { toast('No videos scanned yet', 'warn'); return; }
  if (typeof JSZip === 'undefined') { toast('JSZip not available', 'err'); return; }
  ...
  try {
    ...
    toast(`ZIP downloaded (${n} video${n !== 1 ? 's' : ''})! 🗜`, 'ok');
    if (statusBar) statusBar.textContent = `Downloaded ${n} video${n !== 1 ? 's' : ''}`;
  } catch (e) {
    toast(`ZIP failed: ${e.message || 'Unknown error'}`, 'err');
    if (statusBar) statusBar.textContent = 'ZIP failed';
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = '🗜 Download All'; }
  }
}
...
const PLATFORM_PATTERNS = {
  ...
  'flipkart.com':             { name: 'Flipkart',         tag: 'FLP' },
  'noon.com':                 { name: 'Noon',             tag: 'NON' },
  'lazada.':                  { name: 'Lazada',           tag: 'LZD' },
  'myshopify.com':            { name: 'Shopify',          tag: 'SHP' },
  'target.com':               { name: 'Target',           tag: 'TGT' },
  'costco.com':               { name: 'Costco',           tag: 'CST' },
  'bestbuy.com':              { name: 'Best Buy',         tag: 'BBY' },
  'homedepot.com':            { name: 'Home Depot',       tag: 'HD' }
};
...
function initBulkTab() {
  ...
  $('bulkDlCsvBtn')?.addEventListener('click',  () => downloadBatchSheet('csv'));
...
function cleanTabTitle(raw) {
  if (!raw) return '';
  const s = raw.trim();
  // These strings appear in the browser tab title when Amazon (or any site)
  // is mid-transaction. We don't want them stored as product titles.
  const BAD = /^(adding to cart|add to cart|added to cart|loading\.\.\.|please wait\.\.\.?|checkout|your amazon\.com cart|amazon\.com shopping cart|cart|processing)/i;
  return BAD.test(s) ? '' : s;
}
...
async function startBulkHunt(tabsOverride) {
  ...
  // Smart Sort: Prioritize fast platforms (Amazon, Walmart, etc) over slow ones
  const platformPriority = {
    'amazon.': 1, 'walmart.com': 1, 'samsclub.com': 1, 
    'target.com': 1, 'costco.com': 1, 'bestbuy.com': 1, 'homedepot.com': 1,
    'aliexpress.': 5, 'temu.': 5, 'faire.com': 5, 'alibaba.com': 5, 'shein.com': 5
  };
  selectedTabs.sort((a, b) => {
    let pA = 3, pB = 3;
    for (const k in platformPriority) { if ((a.url || '').includes(k)) pA = platformPriority[k]; }
    for (const k in platformPriority) { if ((b.url || '').includes(k)) pB = platformPriority[k]; }
    return pA - pB;
  });
  ...
  const flushLoop = async () => {
    ...
    while (BulkState.isHunting || BulkState.pendingFlush.length > 0) {
      if (BulkState.pendingFlush.length > 0) {
        ...
        // Only sleep when there's nothing to flush — exit immediately when done
        await sleep(500);
      }
    }
    BulkState._flushActive = false;
  ...
  try {
    ...
    // Phase 2: download images for all results in a global concurrent pool.
    // Runs AFTER all tabs are scraped so workers aren't blocked waiting for images.
  ...
  // Wait for flush loop to finish — with a 10s safety timeout so we never hang
...
async function checkPendingHunt() {
  chrome.storage.local.get(HUNT_STATE_KEY, (res) => {
    ...
    if (ageMs > 2 * 60 * 60 * 1000) { // older than 2 hours — discard
      chrome.storage.local.remove(HUNT_STATE_KEY);
      return;
    }
    ...
    // Store tabs on BulkState for resume
...
async function scrapeOneTab(tabInfo) {
  ...
  try {
    ...
    if (isTemuTab || isWgsTab) {
      ...
      try {
          ...
          chrome.scripting.executeScript({
            ...
            func: () => {
              ...
              if (isWgs) {
                ...
                // Price — Strategy 3: Shopify inline JSON (price in cents)
                if (!result.price) {
                  for (const s of document.querySelectorAll('script:not([src])')) {
                    const t = s.textContent || '';
                    if (t.length < 20 || t.length > 2000000) continue;
                    const cm = t.match(/"price"\s*:\s*(\d{3,7})(?![.\d])/);
                    if (cm) { const cents = parseInt(cm[1]); if (cents > 0 && cents < 10000000) { result.price = '$' + (cents / 100).toFixed(2); break; } }
                ...
                if (!result.price) {
                  const priceSels = ['.price-item--sale','.price-item--regular','.product__price .price','[class*="product-price"]','.price','[data-product-price]'];
                ...
                document.querySelectorAll('img').forEach(img => {
                  ...
                  const w = img.naturalWidth || img.width || 0; if (w > 0 && w < 120) return;
                  ...
                  let large = src.replace(/_(\d+)x(\d+)(\.[a-z]+)(\?|$)/i,'_1200x$3$4').replace(/_(\d+)x(\.[a-z]+)(\?|$)/i,'_1200x$2$3');
                ...
                if (ogImg && ogImg.content) {
                  const cleanOg = ogImg.content.split('?')[0];
                  if (!result.images.includes(cleanOg)) result.images.unshift(cleanOg);
                }

              } else {
                // ══ TEMU scraper ══
                try {
                  const initData = (typeof __init_data__ !== 'undefined' && __init_data__) || window.__init_data__ || null; // eslint-disable-line no-undef
                  if (initData) {
                    const walkInit = (obj, depth) => {
                      if (!obj || typeof obj !== 'object' || depth > 15) return '';
                      const centKeys = ['sale_price','salePrice','actual_price','actualPrice','promotion_price','promotionPrice','min_price','minPrice'];
                      for (const k of centKeys) {
                        if (k in obj && typeof obj[k] === 'number' && obj[k] > 0) return '$' + (obj[k] >= 100 ? (obj[k]/100).toFixed(2) : obj[k].toFixed(2));
                        if (k in obj && typeof obj[k] === 'string' && /\d/.test(obj[k])) { const m = obj[k].match(/[$£€¥₹₩]?\s*\d+(?:[.,]\d+)?/); if (m) return m[0]; }
                      }
                      for (const k of ['origin_url','original_img_url','img_url','thumbnail_url']) { if (k in obj && typeof obj[k]==='string' && obj[k].startsWith('http')) if (!result.images.includes(obj[k])) result.images.push(obj[k]); }
                      for (const v of Object.values(obj)) { if (v && typeof v==='object') { const r2=walkInit(v,depth+1); if(r2) return r2; } }
                      return '';
                    };
                    const p = walkInit(initData, 0); if (p) result.price = p;
                  }
                } catch (_) {}
                if (!result.price) {
                  for (const s of document.querySelectorAll('script:not([src])')) {
                    const t = s.textContent||''; if (t.length<50||t.length>5000000) continue;
                    if (!/sale_price|actual_price|display_price|price_str|priceStr/.test(t)) continue;
                    const c = t.match(/"(?:sale_price|actual_price|promotion_price|salePrice|actualPrice)"\s*:\s*(\d{2,7})(?![.\d])/); if (c) { const cents=parseInt(c[1]); if(cents>0){result.price='$'+(cents/100).toFixed(2);break;} }
                    const sp = t.match(/"(?:display_price|price_str|priceStr|formatted_price)"\s*:\s*"([^"]+)"/); if (sp) { const pm=sp[1].match(/[$£€¥₹₩]\s*\d[\d.,]*/); if(pm){result.price=pm[0];break;} }
                    const fp = t.match(/"(?:price|salePrice)"\s*:\s*(\d+\.\d{1,2})(?!\d)/); if (fp) { const v=parseFloat(fp[1]); if(v>0){result.price='$'+v.toFixed(2);break;} }
                  }
                }
                if (!result.price) {
                  for (const sel of ['[data-testid="price"]','[data-testid="selling-price"]','[class*="price-sale"]','[class*="priceSale"]','[class*="price-current"]','[aria-label*="price" i]']) {
                    const el = document.querySelector(sel); if (!el) continue;
                    const m = (el.innerText||el.textContent||'').match(/[$£€¥₹₩]\s*\d[\d.,]*/); if (m) { result.price = m[0]; break; }
                  }
                }
                if (result.images.length === 0) {
                  for (const s of document.querySelectorAll('script:not([src])')) {
                    const t = s.textContent||''; if (!t.includes('origin_url')&&!t.includes('img_url')&&!t.includes('goods_gallery')) continue;
                ...
                if (result.images.length === 0) {
                  ...
                  gallery.querySelectorAll('img').forEach(img => {
                    const src = img.src||img.dataset.src||img.dataset.lazySrc||''; if (!src||!src.startsWith('http')) return;
                    const w = img.naturalWidth||img.width||0, hh = img.naturalHeight||img.height||0; if (w>0&&w<100) return;
                    if (/logo|icon|badge|nav|platform|header/i.test(src)) return;
                    const clean = src.replace(/\/thumbnail\/\d+x\d+/,'/origin'); if (!result.images.includes(clean)) result.images.push(clean);
                  });
                }
                const ogImg2 = document.querySelector('meta[property="og:image"]');
                if (ogImg2&&ogImg2.content&&!result.images.includes(ogImg2.content)) result.images.unshift(ogImg2.content);
              }
        ...
        error = execErr?.message === 'temu_exec_timeout' ? 'timeout' : (execErr?.message || 'exec_failed');
        scraped = null;
      }
    } else {
  ...
  // PERF: Image download is now deferred to imageDownloadPhase() which runs
  // AFTER all tabs are scraped. This frees worker slots immediately after
  // scraping so the hunt loop runs at full speed without waiting per image.
  const imagesBase64 = [];
  ...
  // Lightweight stub — must include: images (URLs for Phase 2 download),
  // imagesBase64 (populated by imageDownloadPhase), price (for export sheet)
  BulkState.results.push({
    tabId:       result.tabId,
    url:         result.url,
    title:       result.title,
    price:       result.price,        // ← needed for export
    platform:    result.platform,
    status:      result.status,
    error:       result.error,
    images:      result.images || [], // ← needed for imageDownloadPhase
    imagesBase64: [],                 // ← filled by imageDownloadPhase
...
// Instead of downloading images inline in scrapeOneTab (which blocks each
// worker slot for the full image-download duration), we collect all image
// URLs first, then download them in a global concurrent pool.
//
// Benefit: with 12 parallel scrape workers each taking ~3s, 14 tabs finish
// scraping in ~4s. Image download for all 14 results then runs with 6
// concurrent FETCH_BASE64_BATCH calls (each internally parallel in background.js).
async function imageDownloadPhase() {
  // Collect all results that have image URLs to download
  ...
  const workers = Array.from({ length: CONCUR }, async () => {
    while (cursor < pending.length && !BulkState.isCancelled) {
      const r = pending[cursor++]; // safe: JS is single-threaded between awaits
      if (!r) break;
      try {
        const urls = (r.images || []).slice(0, 7); // max 7 per product
        const batchRes = await msg({ action: 'FETCH_BASE64_BATCH', urls });
        if (batchRes?.success && Array.isArray(batchRes.results)) {
          r.imagesBase64 = batchRes.results
            .filter(x => x?.success && x.base64)
            .map(x => x.base64);
        } else {
          r.imagesBase64 = [];
        }
...
function setBulkRowStatus(tabId, status, opts = {}) {
  ...
  if (metaEl) {
    if (status === 'ok')      metaEl.textContent = `${opts.images || 0} imgs`;
      ...
      // Categorize error into human-readable reason
      const errMap = {
        'cancelled':       'cancelled',
        'no_response':     'timeout',
        'timeout':         'timeout',
...
function toggleBulkPause() {
  ...
  if (!BulkState.isPaused) {
    // Reset the service worker keepalive timer when resuming
    msg({ action: 'START_KEEPALIVE' }).catch(() => {});
  }
}
...
async function finishBulkHunt() {
  ...
  const totalImgs = BulkState.results.reduce((s, r) => s + (r.imagesBase64?.length || 0), 0);
  const totalVids = BulkState.results.reduce((s, r) => s + (r.videos?.length || 0), 0);
...
function renderMasterRows(rows) {
  ...
  const q = ($('masterSearchInp')?.value || '').trim().toLowerCase();
  const display = [...rows].reverse().filter(r =>
    !q ||
  ...
  if (display.length === 0) {
    list.innerHTML = `<div class="bulk-empty"><p>No products match &ldquo;${esc(q)}&rdquo;.</p></div>`;
    updateMasterBulkBar();
    return;
  }
  display.forEach(r => {
...
async function bulkDeleteMasterRows() {
  ...
  if (!confirm(`Delete ${ids.length} product${ids.length === 1 ? '' : 's'} from Master Sheet?\n\n${preview}${more}\n\nThis cannot be undone.`)) return;
...
async function reHuntMasterRow(rowId) {
  ...
  // Guard: strip transient browser states ("Adding to Cart…", "Loading…") before
  // overwriting the stored title — same filter used in cleanTabTitle() / BAD_TITLE.
  const BAD_REHUNT_TITLE = /^(adding to cart|add to cart|added to cart|loading\.\.\.|please wait\.\.\.?|checkout|your amazon\.com cart|amazon\.com shopping cart|cart|processing)/i;
  const safeTitle = (scraped.title && !BAD_REHUNT_TITLE.test(scraped.title.trim()))
    ? scraped.title.trim()
    : '';
  ...
  const updatedRows = rows.map(r => {
    ...
    return {
      ...r,
      title:        safeTitle || r.title,  // keep old title if new one is bad/empty
...
// New installs get these off via background.js defaults.
// Existing users who never set them: treated as off here too.
...
function buildRichSheetData(rows) {
  ...
  // ── Sort rows by platform so same-platform products are grouped ──
  const PLATFORM_ORDER_SHEET = [
    'walmart', "sam's club", 'samsclub', 'faire', 'amazon', 'ebay', 'etsy',
    'aliexpress', 'alibaba', 'temu', 'daraz', 'shein'
  ];
  const getPlatformKey = (row) => {
    const p = (row.platform || row.category || '').toLowerCase().trim();
    const idx = PLATFORM_ORDER_SHEET.findIndex(name => p.includes(name));
    return idx >= 0 ? String(idx).padStart(3, '0') + '_' + p : '999_' + p;
  };
  const sortedRows = [...rows].sort((a, b) => {
    const ka = getPlatformKey(a), kb = getPlatformKey(b);
...
function downloadPdfCatalog(rows, titleText) {
  ...
  const cards = rows.map(r => {
    ...
    const imgs = (r.imageUrls || r.images || []).slice(0,5).map(u =>
      '<a href="' + esc(u) + '" style="font-size:10px;color:#0563C1;word-break:break-all;display:block">' + esc(u) + '</a>'
    ).join('');
    return '<div class="card" style="background:' + bg + '">'
      + '<div class="card-header">'
      + (r.url ? '<a class="title" href="' + esc(r.url) + '">' + esc(r.title||r.url) + '</a>'
               : '<span class="title">' + esc(r.title||'Untitled') + '</span>')
      + '</div>'
      + '<div class="meta">'
      + (r.platform||r.category ? '<span class="badge">' + esc(r.platform||r.category) + '</span>' : '')
      + (r.price ? '<span class="price">' + esc(r.price) + '</span>' : '')
      + (r.scrapedAt ? '<span class="date">' + new Date(r.scrapedAt).toLocaleDateString() + '</span>' : '')
      + '</div>'
      + (r.description ? '<p class="desc">' + esc(r.description).slice(0,300) + (r.description.length>300?'...':'') + '</p>' : '')
      + (imgs ? '<div class="imgs"><strong>Images:</strong>' + imgs + '</div>' : '')
      + (r.videoUrl ? '<div class="vid"><strong>Video:</strong> <a href="' + esc(r.videoUrl) + '">' + esc(r.videoUrl) + '</a></div>' : '')
      + '</div>';
  }).join('');

  const html = '<!DOCTYPE html><html><head><meta charset="utf-8">'
    + '<title>' + esc(titleText||'ZHunter PRO Catalog') + '</title>'
    + '<style>'
    ...
    + '</style></head><body>'
    + '<h1>' + esc(titleText||'ZHunter PRO Catalog') + ' (' + rows.length + ' products)</h1>'
    + cards + '</body></html>';

  // Use blob + chrome.downloads instead of window.open (window.open closes the popup)
  ...
  if (typeof chrome !== 'undefined' && chrome.downloads) {
    ...
    // Fallback for sidepanel context
    const a = document.createElement('a');
    a.href = blobUrl;
    a.download = filename;
    document.body.appendChild(a);
...
function downloadCsvRich(rich, filename) {
  ...
  document.body.appendChild(a);
  a.click();
  setTimeout(() => { URL.revokeObjectURL(a.href); a.remove(); }, 3000); // raised from 100ms — give browser time to start download
}
...
function downloadBatchSheet(format) {
  ...
  if (format === 'csv') {
    downloadCsvRich(rich, buildFilename('csv'));
    toast('CSV downloaded (' + rows.length + ' products)', 'ok');
  } else {
    downloadXlsxRich(rich, buildFilename('xlsx'));
    toast('XLSX downloaded (' + rows.length + ' products)', 'ok');
  }
}

async function downloadMasterSheet(format) {
  const allRows = await getMasterRows();
  if (allRows.length === 0) { toast('Master sheet is empty', 'warn'); return; }
  const selectedCount = MasterUIState.selectedIds.size;
  const rows = selectedCount > 0
    ? allRows.filter(r => MasterUIState.selectedIds.has(r.id))
    : allRows;
  const label = selectedCount > 0 ? `${rows.length} selected` : `${rows.length} rows`;
  if (format === 'pdf') {
  ...
  if (format === 'xlsx') {
    ...
    toast('Unknown format: ' + format, 'warn');
    return;
  }
  toast('Master ' + format.toUpperCase() + ' downloaded (' + label + ')', 'ok');
}
...
async function copyMasterSheetToClipboard() {
  ...
  for (let i = 1; i < rich.aoa.length; i++) {
    const row = rich.aoa[i];
  ...
  try {
    ...
    const btn = $('masterCopyTsvBtn');
    if (btn) {
      btn.textContent = '📋 Copied!';
      setTimeout(() => { if (btn) btn.textContent = '📋 Copy (Sheets)'; }, 2500);
    }
    toast('Copied to clipboard!', 'ok');
  } catch (e) {
...
async function downloadBatchZip() {
  ...
  try {
    ...
    // Per-product folders — sorted by platform so same-platform products are grouped
    const PLAT_ORDER_ZIP = [
      'walmart', "sam's club", 'samsclub', 'faire', 'amazon', 'ebay', 'etsy',
      'aliexpress', 'alibaba', 'temu', 'daraz', 'shein'
    ];
    const zipPlatKey = (row) => {
      const p = (row.platform || '').toLowerCase();
      const idx2 = PLAT_ORDER_ZIP.findIndex(n => p.includes(n));
      return idx2 >= 0 ? String(idx2).padStart(3, '0') + '_' + p : '999_' + p;
    };
    const sortedZipRows = [...rows].sort((a, b) => {
      const ka = zipPlatKey(a), kb = zipPlatKey(b);
      return ka < kb ? -1 : ka > kb ? 1 : 0;
    });
    sortedZipRows.forEach((r, i) => {
