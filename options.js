...
function initBulkSettings() {
  ...
  // ── Column toggles grid ──
  const ALL_QUEUE_COLS_OPT = [
    { key: 'folderNumber',  label: 'Folder Number', required: true },
    { key: 'title',         label: 'Product Title', required: true },
    { key: 'link',          label: 'Product Link', required: true },
    { key: 'sourcingPrice', label: 'Sourcing Price', required: true },
  ...
  const renderColGrid = () => {
    ...
    const allCols = [...ALL_QUEUE_COLS_OPT, ...customCols];
    
    allCols.forEach(col => {
      const isCustom = customCols.some(c => c.key === col.key);
      const checked = col.required || prefs[col.key] !== false;
      ...
      item.addEventListener('click', async (e) => {
        ...
        newPrefs[col.key] = !(newPrefs[col.key] !== false);
          const ok = await saveSettings({ productSheetColumns: newPrefs });
          if (ok) {
            currentSettings.productSheetColumns = newPrefs;
          renderColGrid();
          if (colStatus) {
  ...
  if (customColInput && addCustomColBtn) {
    addCustomColBtn.addEventListener('click', async () => {
      ...
      if (customCols.some(c => c.label.toLowerCase() === label.toLowerCase()) || 
          ALL_QUEUE_COLS_OPT.some(c => c.label.toLowerCase() === label.toLowerCase())) {
